...
export function ProjectDetailClient({ project }: { project: Project }) {
  const { t } = useLanguage();
  const pd = translations.projectDetail;
      ...
      <header className="relative pt-8 pb-16 bg-secondary/30 border-b border-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Button variant="ghost" asChild className="mb-8 -ml-4 gap-2 text-muted-foreground hover:text-foreground">
            <Link href="/projects">
              <ArrowLeft className="w-4 h-4" /> {t(pd.back)}
            </Link>
          </Button>
              ...
              {project.liveUrl && (
                <Button asChild className="gap-2 rounded-full">
                  <a href={project.liveUrl} target="_blank" rel="noreferrer">
                    <ExternalLink className="w-4 h-4" /> {t(pd.visitSite)}
                  </a>
                </Button>
              )}
              {project.githubUrl && (
                <Button variant="outline" asChild className="gap-2 rounded-full">
                  <a href={project.githubUrl} target="_blank" rel="noreferrer">
                    <GithubIcon className="w-4 h-4" /> {t(pd.viewSource)}
                  </a>
                </Button>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="md:col-span-2 space-y-16">
            <ScrollReveal delay={0.1}>
              <section>
                <h2 className="text-2xl font-bold mb-4">{t(pd.overview)}</h2>
            ...
            <ScrollReveal delay={0.2}>
              <section>
                <h2 className="text-2xl font-bold mb-6">{t(pd.challenges)}</h2>
                <ul className="space-y-4">
                  {project.challenges.map((challenge, idx) => (
                    <li key={idx} className="flex gap-4 p-4 glass rounded-xl border border-red-500/10">
                      ...
                      <p className="text-muted-foreground pt-1">{t(challenge)}</p>
                    </li>
                  ))}
                </ul>
              </section>
            ...
            <ScrollReveal delay={0.4}>
              <section>
                <h2 className="text-2xl font-bold mb-6">{t(pd.results)}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {project.results.map((result, idx) => (
                    <div key={idx} className="p-6 glass rounded-xl border-l-4 border-l-emerald-500">
                      <p className="text-foreground font-medium">{t(result)}</p>
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="md:col-span-1 space-y-8">
            <ScrollReveal delay={0.2}>
              <div className="sticky top-24 glass p-6 rounded-2xl border-border/50">
                <h3 className="font-bold text-lg mb-4">{t(pd.techStack)}</h3>
